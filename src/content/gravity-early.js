...
(function () {
  ...
  function notify(type, payload) {
    document.dispatchEvent(
      new CustomEvent("__gravity_bridge__", {
  ...
  function processYouTubeData(data) {
    ...
    console.log(
      `[Gravity Capture] YouTube player response intercepted. Video: ${data.videoDetails?.title}`,
    );
    ...
    for (const format of allFormats) {
      formats.push({
        ...
        isCombined: !!(data.streamingData.formats || []).find(
          (f) => f.itag === format.itag,
        ),
      });
    }
  ...
  function captureResponseData(url, buffer, source) {
    ...
    console.log(
      `[Gravity Capture] Captured segment for ${trackKey} (${source}): ${cloned.byteLength} bytes. Total captured: ${track.totalSize} bytes.`,
    );
  }
  ...
  const blobToMsId = new Map(); // blob URL → MediaSource ID
  ...
  OriginalMediaSource.prototype.addSourceBuffer = function (mimeType) {
    ...
    // Get the MediaSource ID for this SourceBuffer
    const msData = mediaSourceMap.get(this);
    const msId = msData ? msData.id : null;
    ...
    console.log(
      `[Gravity Early] SourceBuffer created: ${trackKey} (${mimeType})`,
    );
    return sb;
  };
        ...
        if (data && (data.slice || data.buffer)) {
          ...
          if (trackKey && capturedTracks.has(trackKey)) {
            ...
            if (totalCapturedBytes < MAX_CAPTURE_BYTES) {
              ...
              console.log(
                `[Gravity Capture] Captured segment for ${trackKey} (SourceBuffer): ${clonedData.byteLength} bytes. Total captured: ${track.totalSize} bytes.`,
              );
            }
          }
        }
              ...
              clonedResponse
                ...
                .then((buffer) => {
                  if (buffer && buffer.byteLength > 1000) {
                    // Skip tiny responses (errors, pings)
  ...
  try {
    Object.defineProperty(HookedXHR, "UNSENT", {
      value: OrigXMLHttpRequest.UNSENT,
      writable: false,
    });
    Object.defineProperty(HookedXHR, "OPENED", {
      value: OrigXMLHttpRequest.OPENED,
      writable: false,
    });
    Object.defineProperty(HookedXHR, "HEADERS_RECEIVED", {
      value: OrigXMLHttpRequest.HEADERS_RECEIVED,
      writable: false,
    });
    Object.defineProperty(HookedXHR, "LOADING", {
      value: OrigXMLHttpRequest.LOADING,
      writable: false,
    });
    Object.defineProperty(HookedXHR, "DONE", {
      value: OrigXMLHttpRequest.DONE,
      writable: false,
    });
  } catch (e) {
    ["UNSENT", "OPENED", "HEADERS_RECEIVED", "LOADING", "DONE"].forEach(
      (prop) => {
        try {
          HookedXHR[prop] = OrigXMLHttpRequest[prop];
        } catch (e2) {}
      },
    );
  }
  ...
  window.__gravityForceBuffer = async function (options = {}) {
    ...
    if (!duration || isNaN(duration) || duration === Infinity) {
      return {
        success: false,
        error: "Cannot determine video duration (live stream?)",
      };
    }

    const { seekStep = 5, seekDelay = 300, autoDownload = true } = options;

    _isAutoBuffering = true;
    _bufferAbortController = { aborted: false };

    console.log(
      `[Gravity HiddenPlayer] Starting auto-buffer for ${duration.toFixed(1)}s video`,
    );
    console.log(
      `[Gravity HiddenPlayer] Settings: step=${seekStep}s, delay=${seekDelay}ms`,
    );
    ...
    try {
      const player =
        document.getElementById("movie_player") || video.parentElement;
      if (player) {
    ...
    function reportProgress(phase, percent, message) {
      const detail = {
        ...
        message,
        capturedMB: (
          (totalCapturedBytes - startingCapturedBytes) /
          1024 /
          1024
        ).toFixed(1),
      ...
      document.dispatchEvent(
        new CustomEvent("gravity:buffer-progress", { detail }),
      );
      ...
      if (overlayDiv) {
        ...
        if (pctEl)
          pctEl.textContent = `${detail.percent}% - ${detail.capturedMB}MB`;
      }

      console.log(
        `[Gravity HiddenPlayer] ${phase}: ${detail.percent}% - ${message} (${detail.capturedMB}MB captured)`,
      );
    }
    ...
    try {
      ...
      if (video.paused) {
        try {
          ...
          console.warn(
            "[Gravity HiddenPlayer] Autoplay blocked, trying click simulation",
          );
          ...
          try {
            ...
            return {
              success: false,
              error:
                "Could not start playback. Click Play on the video first, then try again.",
            };
          }
        }
      }
      ...
      try {
        ...
        if (player && typeof player.setPlaybackQualityRange === "function") {
          ...
          if (qualities.length > 0) {
            ...
            console.log(
              `[Gravity HiddenPlayer] Setting quality: ${highest} (available: ${qualities.join(",")}`,
            );
        ...
        console.warn(
          "[Gravity HiddenPlayer] Could not set quality:",
          e.message,
        );
      }
      ...
      while (seekPosition < duration - 1) {
        ...
        if (!seeked) {
          ...
          if (stuckCount > 10) {
            console.warn(
              "[Gravity HiddenPlayer] Stuck — breaking out to try download with partial data",
            );
            break;
          }
        } else {
        ...
        const bufferedPercent = (
          (getBufferedDuration(video) / duration) *
          100
        ).toFixed(0);
        reportProgress(
          "buffering",
      ...
      console.log(
        `[Gravity HiddenPlayer] Buffering complete! Captured ${capturedMB}MB, ${bufferedPercent}% buffered`,
      );
      ...
      if (autoDownload && newCapturedBytes > 50000) {
        ...
        reportProgress(
          "complete",
          100,
          result.success ? "Download started!" : result.error,
        );
      ...
      reportProgress(
        "complete",
        100,
        `Buffering complete: ${capturedMB}MB captured`,
      );
      _isAutoBuffering = false;
      return {
        success: true,
  ...
  function getCapturedTracksSummary() {
    ...
    for (const [key, track] of capturedTracks) {
      tracks.push({
        ...
        mediaSourceId: track.mediaSourceId || null,
      });
    }
    return tracks;
  }
  ...
  function findWebMStart(buffer) {
    ...
    for (let i = 0; i < len - 4; i++) {
      if (
        view[i] === WEBM_MAGIC[0] &&
        view[i + 1] === WEBM_MAGIC[1] &&
        view[i + 2] === WEBM_MAGIC[2] &&
        view[i + 3] === WEBM_MAGIC[3]
      ) {
        return i;
      }
    }
      ...
      if (mediaStart >= 0) {
        ...
        if (mediaData.byteLength > 0) {
          ...
          if (!initSegmentFound) {
            ...
            console.log(
              `[Gravity Early] First segment starts with '${firstFourCC}' box at offset ${mediaStart}`,
            );
        ...
        console.log(
          `[Gravity Early] Segment ${i}: no media header found, skipping (${buffer.byteLength} bytes)`,
        );
      }
    }
  ...
  window.__gravityDownloadCapturedVideo = function (
    preferVideo = true,
    targetBlobUrl = null,
  ) {
    const tracks = getCapturedTracksSummary();
    console.log(
      "[Gravity Early] Download requested. Captured tracks:",
      tracks,
      "targetBlobUrl:",
      targetBlobUrl,
    );
    ...
    if (tracks.length === 0) {
      return {
        success: false,
        error: "No video data captured. Play the video first!",
      };
    }

    // If we have a target blob URL, filter tracks to only those
    // belonging to the same MediaSource (same video element)
    let filteredTracks = tracks;
    if (targetBlobUrl) {
      const targetMsId = blobToMsId.get(targetBlobUrl);
      if (targetMsId) {
        const msFiltered = tracks.filter((t) => t.mediaSourceId === targetMsId);
        if (msFiltered.length > 0) {
          filteredTracks = msFiltered;
          console.log(
            `[Gravity Early] Filtered to ${msFiltered.length} tracks for MediaSource ${targetMsId}`,
          );
        }
      }
    }
    ...
    const cleanTracks = filteredTracks.filter((t) => t.isClean);
    const rawTracks = filteredTracks.filter((t) => !t.isClean);

    console.log(
      `[Gravity Early] Clean MSE tracks: ${cleanTracks.length}, Raw fetch tracks: ${rawTracks.length}`,
    );
    ...
    console.log(
      `[Gravity Early] Downloading track: ${bestTrack.key}, ` +
    ...
    try {
      ...
      if (bestTrack.isClean) {
        ...
        console.log(
          `[Gravity Early] Using ${segmentsToUse.length} clean MSE segments directly`,
        );
      } else {
        // Raw fetch data — needs SABR header stripping
        segmentsToUse = extractMediaData(
          bestTrack.segments,
          bestTrack.mimeType,
        );
        if (segmentsToUse.length === 0) {
          return {
            success: false,
            error: "Could not extract media data from captured segments",
          };
        }
        console.log(
          `[Gravity Early] Extracted ${segmentsToUse.length} segments after SABR stripping`,
        );
      }
      ...
      // Determine filename
      let filename = "Gravity_video_" + Date.now();
      const ytData =
        extractedVideoData.length > 0
          ? extractedVideoData[extractedVideoData.length - 1]
          : null;
      if (ytData) {
  ...
  console.log(
    "[Gravity Early] MSE & network hooks installed (with segment capture)",
  );
})();
